...
class ShowEntity extends Component {
  ...
  componentDidUpdate() {
    const { baseUrl, history, destroyed, destroyCleanup } = this.props;
    ...
    return (
      <LayoutWrapper>
        <div>
          <Link to={baseUrl}>
            <Button>
              <IntlMessages id="rest.index" values={{ name: "" }} />
            </Button>
          </Link>
          <Link to={`${baseUrl}/${primaryKey}/edit`}>
            <Button>
              <IntlMessages id="rest.edit" values={{ name: "" }} />
            </Button>
          </Link>
          <Button
            type="danger"
        ...
        <Box>
          <Spin spinning={fetching}>
            <div key={primaryKey}>{this.props.children}</div>
          </Spin>
        </Box>
      </LayoutWrapper>
    );
